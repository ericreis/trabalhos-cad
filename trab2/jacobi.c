...
*/
{
  ...
  #pragma omp parallel for private(j, i, x, y, u_true) reduction(+:error_norm, u_true_norm)
  for ( j = 0; j < n; j++ )
  {
    for ( i = 0; i < m; i++ )
    {
...
*/
    #pragma omp parallel for private(j, i, error) reduction(+:error_norm)
    for ( j = 0; j < n; j++ )
    {
      for ( i = 0; i < m; i++ )
      {
...
*/
  ...
  #pragma omp parallel for private(i, j) reduction(+:f_norm)
  for ( j = 0; j < n; j++ )
  {
    for ( i = 0; i < m; i++ )
    {
